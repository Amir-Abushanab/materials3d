...
`rod`, `disc`, `prism`, `hex`, `cone`, `sphere`, `ring`, `arrow`, `droplet`, `blob`, `slab`,
`path` (an SVG outline, extruded) and `model` (a `.glb`). Flat-profile shapes take through `cuts`
of kind `rect` or `circle`. A `scatter` block generates a row of shapes from a seed in place of
`items`.

A `model` shape names its file in `shape.model`, as a URL or a `data:` URI:

```json
{ "shape": { "kind": "model", "model": "/hero.glb", "r": 2.6 } }
```

The mesh is centred on its own bounding box and scaled until its longest half-extent is `r`, so a
file authored in millimetres and one authored in metres land at the same size. `.glb` only, the
single-file form; materials, textures, rigs and animation in it are ignored, since the material
comes from the config. Up to 250k triangles.
...
[`gallery/community/knot.json`](gallery/community/knot.json) is a `model` scene: a `.glb` in glass
between two primitives, all sharing one lamp field and one post stack.
...
- No CSG. Intersecting or boolean shapes need pre-authored geometry, which `model` is the way to bring in.
- `model` reads geometry only, and no compressed `.glb`: Draco and meshopt files are refused by name rather than half-read. Quantized and sparse accessors are read.
- A `model` can be a `beam` target: the mesh is cut at the beam's plane and the largest contour traced. Only that one, so a beam aimed through a lens opening or a gap bends light the mesh draws as empty.
