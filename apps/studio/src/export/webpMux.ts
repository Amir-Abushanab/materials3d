...
function frameImageChunks(file: Uint8Array): { data: Uint8Array; hasAlpha: boolean } {
  ...
  while (off + 8 <= file.length) {
    ...
    if (cc === "ALPH" || cc === "VP8 " || cc === "VP8L") {
      ...
      // A lossless frame carries alpha inside its own bitstream, not as an ALPH chunk: the
      // VP8L header is a 0x2F signature byte then a u32 of 14-bit width-1, 14-bit height-1,
      // 1-bit alpha_is_used. (Chrome's lossy toBlob path always emits ALPH, so this branch is
      // for lossless inputs.)
      if (cc === "VP8L" && off + 13 <= file.length && (u32le(file, off + 9) >>> 28) & 1) {
        hasAlpha = true;
      }
    }
    off = end;
