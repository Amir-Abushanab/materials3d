...
.m3d-scrolltest {
  ...
  scrollbar-width: thin; scrollbar-color: rgba(20,20,26,0.35) transparent;
  opacity: 0; transition: opacity .18s ease;
}
.m3d-scrolltest.is-open { opacity: 1; }
.m3d-scrolltest__bar {
...
.m3d-scrolltest__label { flex: 1 1 auto; min-width: 0; }
.m3d-scrolltest__label b { color: #9ad0ff; font-weight: 600; }
.m3d-scrolltest__pct {
  flex: none; font-variant-numeric: tabular-nums; font-weight: 600;
  min-width: 3.1em; text-align: right; color: #9ad0ff;
}
.m3d-scrolltest__close {
...
.m3d-scrolltest__close:hover { background: rgba(255,255,255,0.14); }
.m3d-scrolltest__track {
  position: absolute; left: 0; right: 0; bottom: -1px; height: 2px;
  background: rgba(255,255,255,0.08);
}
.m3d-scrolltest__fill {
  height: 100%; width: 0%;
  background: linear-gradient(90deg, #6ea8fe, #9b7bff);
}
.m3d-scrolltest__runway { position: relative; }
.m3d-scrolltest__mark {
...
@media (prefers-reduced-motion: reduce) {
  .m3d-scrolltest { transition: none; }
}
`;

/** Overlay that turns the scene preview into a scrollable surface driving the scroll input. */
export class ScrollTestOverlay {
  ...
  open(): void {
    ...
    const marks = [0, 25, 50, 75, 100]
      .map((p) => {
        ...
        return `<div class="m3d-scrolltest__mark" style="top:${p}%">${label}</div>`;
      })
      .join("");
    const root = document.createElement("div");
    root.className = "m3d-scrolltest";
    root.setAttribute("role", "region");
    root.setAttribute("aria-label", "Scroll test");
    root.innerHTML =
      `<div class="m3d-scrolltest__bar">` +
      `<span class="m3d-scrolltest__label">Scroll here to drive your <b>scroll</b> / <b>scroll&nbsp;velocity</b> reactions</span>` +
      `<span class="m3d-scrolltest__pct">0%</span>` +
      `<button type="button" class="m3d-scrolltest__close" aria-label="Close scroll test">Close ✕</button>` +
      `<div class="m3d-scrolltest__track"><div class="m3d-scrolltest__fill"></div></div>` +
      `</div>` +
      `<div class="m3d-scrolltest__runway" style="height:${RUNWAY * 100}%">${marks}</div>`;
    this.stage.appendChild(root);
    this.root = root;
    this.fill = root.querySelector<HTMLElement>(".m3d-scrolltest__fill") ?? undefined;
    this.pct = root.querySelector<HTMLElement>(".m3d-scrolltest__pct") ?? undefined;
    root
      .querySelector<HTMLButtonElement>(".m3d-scrolltest__close")
  ...
  close(): void {
    ...
    this.renderer.setScrollPreview(0); // return the scene to rest when the test closes
  }
