/**
 * Hover hints for the control-panel labels.
 *
 * A hinted label gets a dotted underline and a "help" cursor; hovering it (or keyboard-focusing
...
const CONTROL_HINTS: Record<string, string> = {
  // --- Actions ---
  "🎲 randomize scene":
    "Rerolls the whole look within tasteful bounds: lamps, optics, framing, motion and post. Shape kinds are left alone, since the shape set is the scene's identity. Undo brings the old one back.",
  "🎲 shuffle lamps":
  ...
  "🔗 Copy share link":
    "Copies a URL with the whole config packed into its #hash. No server involved. A backdrop image or video chosen from disk is left out, since a link cannot carry a file; save the config to keep it.",
  "⟨⟩ Get code…":
    "The current scene as a ready-to-paste snippet: React, web component, vanilla, CDN or raw JSON. The code tabs include only values that differ from the defaults; the JSON tab is the full config.",
  "✏ Edit config…":
  ...
  "ease in on load":
    "Ramps the animation up over about a second on load instead of starting at full speed. Live playback only; exports and posters are untouched.",
  ...
  // --- Image / video ---
  "🖼 Choose image…":
    "A backdrop image from disk. Stored as a data URI so it survives a save and reload; large files make large configs, and that is the trade. Left out of share links, which cannot carry a file.",
  "🎞 Choose video…":
  ...
  // --- Lamps ---
  gain: "Scales the whole lamp field's coverage before the gate. More gain means more of the frame carries colour.",
  "gate lo":
  ...
  "on backdrop":
    'How much of the lamp field shows on the backdrop itself, in the gaps between shapes. If colour appears only inside glass the eye reads it as tint; a faint presence outside (about 0.05) is what sells "the colour is behind".',
  palette:
  ...
  // --- Camera ---
  fov: "Vertical field of view in degrees. Keep it long (about 12): with a wide fov, rotation about a horizontal axis reads as tumbling instead of foreshortening.",
  "aspect fit":
  ...
  range: "Depth either side of focus that stays sharp. Narrow is a thin slice of crisp glass.",
  aperture:
    "Maximum blur circle in pixels for out-of-focus depths. Works with 'range': wide aperture plus narrow range blurs almost everything.",
  bloom:
  ...
  sides:
    "Lathe segments. High is round; six is a hexagonal prism, and most of the prism family is just this number.",
  ...
  amount:
    "How far the motion moves the shape: the bob height of a drift, or the squash of a wobble, in world units. Skewer and spin ignore it.",
  "↻ apply to all shapes":
  ...
  density: "Absorption strength along that path. Higher pulls deeper colour out of the same glass.",
  tint: "Gives the shape its own absorption colour instead of borrowing the lamps behind it.",
  ior: "Refraction strength. 1.45 is about glass; higher bends the sampled background harder.",
  ...
  "edge (F82)":
    'Measured reflectance near the silhouette (about 82 degrees). Real metals desaturate there; plain Schlick cannot, and "gold that is uniformly gold" is what reads as CG. Empty means plain Schlick.',
  metal:
    "Preset F0/F82 pairs measured from real conductors. 'custom' means the colour was hand-picked.",

  // --- Output ---
  size: "Exact pixel size every export renders at, independent of the preview. GPU-heavy sizes are marked in the list: four passes cost about four times the fill rate of a single-pass renderer.",
  ...
  "actual size":
    "Shows the export frame at one export pixel per CSS pixel, and renders the preview at the export size while it is on. Large sizes cost the same as the export itself, live.",
  "still format":
    "File type for stills. PNG is lossless (and what the calibrate tool reads); WebP and JPEG are smaller.",
  "video format":
    "WebM and MP4 record in real time. Animated WebP and GIF are walked frame by frame through seek(), so they are frame-exact and reproducible, and WebP keeps alpha, which the video formats cannot.",
  seconds: "Clip length. Setting a loop syncs this to it, so recordings cut seamlessly.",
  fps: "Frames per second of the recording. GIF stores whole centiseconds per frame, so 24 plays back as 25; the readout below says what the file will do.",
 ...
 * (folder titles normalized: live counts and per-item numbers stripped, so "Lamps · 10" reads as
 * "Lamps" and "lamp 3" as "lamp").
 */
const FOLDER_HINTS: Record<string, string> = {
  ...
  "Backplate distance":
    "How far behind the scene the lamp plate hangs. A critical knob: far back, every shape lenses the whole gradient into rainbow banding; close in (about -3) refraction reads as distortion of one continuous field.",
  "Camera distance": "Dolly distance from the look-at point, in world units.",
  // "kind": motion vs shading model (bare "kind" is the shape's geometry, above).
  "Motion kind":
  ...
  "lamp radius":
    "This lamp's Gaussian radius, in plate space (the backplate's 0 to 1 coordinates).",
  "lamp x":
    "Horizontal position in plate space. 0 to 1 spans the plate; a little outside is allowed.",
 ...
 *  implying the whole control is hoverable-for-help, so they get a small info glyph instead, a
 *  separate, deliberate target, so hovering to read never competes with clicking to act.
...
/** "Lamps · 10" to "Lamps", "lamp 3" to "lamp": one FOLDER_HINTS key covers every instance. */
 ...
 *
 * A button's label may carry a suffix its key does not: the still export retitles itself per
 * format ("Save still (.webp)"), so buttons also match on the longest key their label starts
 * with. Only buttons: a label like "haze top" must never inherit "haze".
 */
function lookupHint(label: string, row: HTMLElement, isButton: boolean): string | undefined {
  ...
  const exact = CONTROL_HINTS[label] ?? HINTS_BY_RENDERED_TEXT.get(label);
  if (exact || !isButton) return exact;
  let best: string | undefined;
  let bestLength = 0;
  for (const [key, text] of HINTS_BY_RENDERED_TEXT) {
    if (key.length > bestLength && label.startsWith(key)) {
      best = text;
...
/**
 * Give a button its own info glyph and anchor the hint there, so hovering anywhere else on the
 * button (the whole point of which is to be clicked) stays quiet.
 */
function attachGlyph(btnLabel: HTMLElement, label: string, text: string): void {
  ...
  info.addEventListener("pointerenter", () => show(info, text));
  info.addEventListener("pointerleave", scheduleHide);
  btnLabel.append(info);
}

/** The element a row's hint hangs off, read live: a button's glyph is rebuilt when it is retitled. */
function anchorOf(row: HTMLElement): HTMLElement | null {
  return (
    row.querySelector<HTMLElement>(".g3-hint-info") ?? row.querySelector<HTMLElement>(".tp-lblv_l")
  );
}

/**
 * Mark every hinted label in `container` with the underline affordance and wire its hover/focus
 * triggers. Idempotent per row (safe to re-run after each panel rebuild, and after a lazily built
 * shape folder fills in; Tweakpane hands us fresh DOM each time, so old listeners are discarded
 * with the old nodes). Run AFTER applyIcons, which swaps leading emoji for SVGs: the icon
 * contributes no text, so labels read clean here.
 */
export function applyControlHints(container: HTMLElement): void {
  ...
  container.querySelectorAll<HTMLElement>(".tp-lblv").forEach((row) => {
    ...
    if (row.dataset.g3Hinted) {
      // A retitled button takes its glyph away with the old text (Tweakpane rewrites the title
      // element); put it back. The row's focus listeners resolve their anchor live, so they hold.
      if (btnLabel && !btnLabel.querySelector(".g3-hint-info")) {
        const text = lookupHint(label, row, true);
        if (text) attachGlyph(btnLabel, label, text);
      }
      return;
    }
    const text = lookupHint(label, row, btnLabel !== null);
    if (!text) return;

    row.dataset.g3Hinted = "1";
    if (btnLabel) {
      attachGlyph(btnLabel, label, text);
    } else {
      keyEl.classList.add("g3-has-hint");
      keyEl.addEventListener("pointerenter", () => show(keyEl, text));
      keyEl.addEventListener("pointerleave", scheduleHide);
    }
    ...
    row.addEventListener("focusin", () => {
      ...
      if (keyboardModality && anchor) show(anchor, text);
    });
