 ...
 *   node scripts/tsl-parity.mjs --dump post     # also write both frames of one case to renders/
 *   node scripts/tsl-parity.mjs --help
 *
...
const USAGE = `usage: pnpm tsl:parity [--dump <case>] [--out <dir>]

Renders every case through both engines and reports the worst per-channel difference. Exits
non-zero when any case differs by more than one quantisation level or fails to render.

  --dump <case>  also write the case's two frames as parity-<case>-{glsl,tsl}.png (env DUMP works too)
  --out <dir>    where the dump goes (default renders/)`;

await run(async (defer) => {
  const args = parseArgs(process.argv.slice(2), { dump: "", out: "" });
  if (args.help) {
    console.log(USAGE);
    return;
  }
  requireBuild({ dist: true });
  const DUMP = args.dump || process.env.DUMP || "";

  const server = await serve({
    html: `<!doctype html><meta charset=utf-8><style>html,body{margin:0}</style>`,
  });
  defer(server.close);
  // Shader compile failures arrive as console errors, not exceptions, and a failed GLSL reference
  // renders black, which looks exactly like a broken TSL port; the harness prints them as they come.
  const { page, check, close } = await launch({ width: 256, height: 256 });
  defer(close);
  ...
  const results = await page.evaluate(
    ...
    vec3 rampColor(vec2 p){
      ...
      // Transcribed once from `shaders.ts` and interpolated wherever a case needs them, so the copies
      // that several cases used to carry cannot drift from one another.
    ...
    vec3 tonemapNeutral(vec3 v){
      ...
      void main(){
        ...
        tsl: () =>
          passes.bloomCompositePass(gpuTex, gpuTex1, gpuTex2, TSL.float(0.35), TSL.float(0)),
      };
    ...
    vec3 thinFilm(float ndv, float ior, float film, float irid){
      ...
      void main(){
        ...
        tsl: () =>
          TSL.vec4(
            brdf.thinFilm(TSL.uv().x, TSL.float(1.5), TSL.uv().y.mul(900), TSL.float(0.8)),
            1,
          ),
      };
    ...
    vec3 spectralWeight(float t){
      ...
      void main(){
        ...
        tsl: () =>
          TSL.vec4(
            TSL.vec3(
              backdropNodes.shadowContrastCurve(
                TSL.uv().x,
                TSL.uv().y.mul(8).add(1),
                TSL.float(0.9),
              ),
            ),
            1,
  ...
  for (const r of results) {
    ...
    await writeDataUrl(resolve(dir, `parity-${r.name}-glsl.png`), r.glPng);
    await writeDataUrl(resolve(dir, `parity-${r.name}-tsl.png`), r.gpuPng);
    console.log(`  dumped ${dir}/parity-${r.name}-{glsl,tsl}.png`);
  }
  return failed === 0;
});
